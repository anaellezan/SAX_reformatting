...
def reformat_to_short_axis(path, ct_im, mask_lvendo, mask_lvwall, mask_rvepi, save=False, isotropic=1):
    ...
    isotropic_resolution = isotropic           # if 0, output spacing depends on initial data size (covers entire image)
    ...
    # Input filenames
    ifilename_ct = path + ct_im
    ifilename_lvendo = path + mask_lvendo
    ifilename_lvwall = path + mask_lvwall
    ifilename_rvepi = path + mask_rvepi
    ...
    # Output
    ofilename_ct = path + ct_im[0:-4] + '-sax.mha'
    R_sax_filename = path + ct_im[0:-4] + '-R-matrix-sax.txt'
    ...
    ###  Find rotations
    R = compute_rotation_to_sax(path, path + mask_lvendo,
                                path + mask_lvwall, path + mask_rvepi,
                                reference_image, reference_center, sax_origin, delete_intermediate=not(save))
